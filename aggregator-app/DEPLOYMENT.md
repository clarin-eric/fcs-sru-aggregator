...
The following subsections will only focus on the `aggregator.*` parameters. Other parameters are explained in the DropWizard documentation.

### Endpoint configuration

Endpoints will be loaded from the URL configured in `aggregator.scan.centreRegistryUrl`. Optional side-loading endpoints (for testing) can be done via `endpointOverrides`.

```yaml
aggregator:
  scan:
    # CLARIN centre registry
    centreRegistryUrl: https://centres.clarin.eu/

  # optional, can also be commented out
  endpointOverrides:
    - url: https://clarin.ids-mannheim.de/digibibsru-new
      isCQL: true
...
```yaml
# both (both CQL/FCS) `endpointOverrides`
aggregator:
  endpointOverrides: 
...
# backwards compatibility (simple string/URL)
   - url: https://clarin.ids-mannheim.de/digibibsru-new
   - url: http://localhost:8080/korp-endpoint/sru
   - url: https://spraakbanken.gu.se/ws/fcs/2.0/endpoint/korp/sru
...
The `aggregator.scan.cachedResourcesFile` and `aggregator.scan.cachedResourcesBackupFile` are used to store the current and previous `scan`/`explain` result. The same content is exposed via the REST API.

### WebLicht

The default WebLicht configuration is as follows. The only setting to change based on deployment is `weblicht.exportServerUrl` or `SERVER_URL` which should be the publicly available domain (and path if prefixed) the Aggregator runs at. This is required since TCF exports for [WebLicht](https://weblicht.sfs.uni-tuebingen.de) are temporarily stored at the Aggregator and therefore need to be accessible to process results.
...
aggregator:
  ...
  serverUrl: ${SERVER_URL:-https://contentsearch.clarin.eu/}
```

The environment variable `SERVER_URL` is by default used by `SERVER_URL` and in `weblicht.exportServerUrl`.
